...
#commented for check git pull1444
...
def clientthread():
    ...
    while True:
        try:
            ...
            #print (response);
            #print ("Card read UID: {0},{1},{2},{3},{4}".format(*response));
            #response+='\r' + '\n';
            #response=response.replace(b'\x02', b'')
            #data=int.from_bytes(response, byteorder='big')
            data=response
            print (data);
            ...
            try:
                ...
                last_dev=last_dev.replace("\n", "")
                print(last_dev);          
                ser = serial.Serial(last_dev, 19200)
            except:
                print("");
